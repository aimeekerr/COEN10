...
int main (void)
{
	...
	switch(pick)
	{
		...
		{
			if(flag1!=0)
				printf("Priority has already been used today");
			else
			{
				priority();
			}
...
void priority(){
	...
	char prio[20];
	char phone1[11];
	printf("Welcome to priority mode, please enter your name!\n");
	scanf("%s",name);
	for(i=0;i<5;i++)
	{
		if(strcmp(array[i],name)==0)
	...
	{
		size[0]=size1;
		printf("%s's lesson at 5 has been canceled\n",array[4]);
		for(i=4;i>0;i--)
		{
		...
		strcpy(array[0],name);
		size[0]=size1;
		strcpy(phone[0],phone1);
		printf("Your lesson will be at 1!");
		flag1++;
	}
